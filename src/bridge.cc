...
bool Bridge::EnsureConnectedLocked() {
    ...
    if (connect(socket_fd_, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) < 0) {
...
bool Bridge::SetLedState(bool led_state) {
    ...
    std::cerr << "[Bridge] -> set_led_state msgid=" << msgid << " value=" << (led_state ? "true" : "false") << '\n';
    ...
    while (remaining > 0) {
        ...
        if (sent < 0) {
            std::cerr << "[Bridge] send(set_led_state) failed msgid=" << msgid << ": " << std::strerror(errno) << '\n';
            MarkDisconnectedLocked();
            return false;
        }
        data += sent;
    ...
    std::cerr << "[Bridge] set_led_state msgid=" << msgid << " sent, waiting for response" << '\n';

    while (true) {
        msgpack::object_handle handle;
        if (!unpacker_.next(handle)) {
            unpacker_.reserve_buffer(kRecvChunk);
            const ssize_t received = recv(socket_fd_, unpacker_.buffer(), kRecvChunk, 0);
            if (received <= 0) {
                std::cerr << "[Bridge] recv(set_led_state) failed msgid=" << msgid << ": " << std::strerror(errno) << '\n';
                MarkDisconnectedLocked();
                return false;
            }
            unpacker_.buffer_consumed(static_cast<std::size_t>(received));
            std::cerr << "[Bridge] recv(set_led_state) bytes=" << received << " msgid=" << msgid << '\n';
            continue;
        }
        ...
        try {
            ...
            if (message_type != 1) {
                std::cerr << "[Bridge] skip non-response frame type=" << message_type << " while waiting msgid=" << msgid << '\n';
            ...
            if (response_msgid != msgid) {
                std::cerr << "[Bridge] skip response for other msgid=" << response_msgid << " while waiting msgid=" << msgid << '\n';
            ...
            std::cerr << "MessagePack decode failed (set_led_state msgid=" << msgid << "): " << e.what() << '\n';
            return false;
        }
    }
}
...
bool Bridge::SetMatrix(const std::array<uint8_t, 104>& pixels) {
    ...
    const std::uint32_t msgid = next_msgid_++;
    std::cerr << "[Bridge] -> set_matrix msgid=" << msgid << " pixels=104 first4=["
    ...
    packer.pack_bin_body(reinterpret_cast<const char*>(pixels.data()), pixels.size());
    ...
    while (remaining > 0) {
        const ssize_t sent = send(socket_fd_, data, remaining, 0);
        if (sent < 0) {
            std::cerr << "[Bridge] send(set_matrix) failed msgid=" << msgid << ": " << std::strerror(errno) << '\n';
            MarkDisconnectedLocked();
            return false;
        }
        data += sent;
    ...
    std::cerr << "[Bridge] set_matrix msgid=" << msgid << " sent, waiting for response" << '\n';
    ...
    while (true) {
        ...
        if (!unpacker_.next(handle)) {
            ...
            const ssize_t received = recv(socket_fd_, unpacker_.buffer(), kRecvChunk, 0);
            if (received <= 0) {
                std::cerr << "[Bridge] recv(set_matrix) failed msgid=" << msgid << ": " << std::strerror(errno) << '\n';
                MarkDisconnectedLocked();
                return false;
            }
            unpacker_.buffer_consumed(static_cast<std::size_t>(received));
            std::cerr << "[Bridge] recv(set_matrix) bytes=" << received << " msgid=" << msgid << '\n';
            continue;
        }
        ...
        try {
            ...
            std::cerr << "MessagePack decode failed (set_matrix msgid=" << msgid << "): " << e.what() << '\n';
            return false;
        }
    }
}