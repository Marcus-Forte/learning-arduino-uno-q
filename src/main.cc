...
class McuServiceImpl final : public McuService::Service {
 public:
	grpc::Status SetLed(grpc::ServerContext* /*context*/, const LedRequest* request,
		...
		if (!bridge_.SetLedState(request->on())) {
			...
			return grpc::Status(grpc::StatusCode::INTERNAL, "Failed to call router socket");
		}

		return grpc::Status::OK;
	}

	grpc::Status SetMatrix(grpc::ServerContext* /*context*/, const MatrixRequest* request,
											 google::protobuf::Empty* /*response*/) override {
		std::cout << "SetMatrix called, pixels_size=" << request->pixels_size() << '\n';
		if (request->pixels_size() != 104) {
			std::cerr << "SetMatrix rejected: expected 104 pixels, got " << request->pixels_size() << '\n';
			return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "pixels must contain exactly 104 values");
		}
		...
		for (int i = 0; i < request->pixels_size(); ++i) {
			...
			if (value > 255u) {
				return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "pixel values must be in range 0..255");
			}
